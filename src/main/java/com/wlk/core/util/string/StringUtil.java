package com.wlk.core.util.string;
...
public class StringUtil extends StringUtilParent {
	...
	public static final String randomString(int length) {
        ...
        char [] randBuffer = new char[length==0?12:length];
	 ...
	 * @return
	 */
	public static final String randomCode() {
		Random randGen = null;
		char[] numbersAndLetters = null;
		if (randGen == null) {
			randGen = new Random();
	 ...
	 * @return
	 */
	public static Date getNDay(Date date,Integer day) {
	...
	//时间格式 2017-3-28 16:16:34
	 public static boolean isDateBefore(Date date2){
	  try{
	   Date date1 = new Date();
	   DateFormat df = DateFormat.getDateTimeInstance();
	   return date1.before(df.parse(df.format(date2)));
	  }catch(ParseException e){
	   System.out.print("[SYS] " + e.getMessage());
	   return false;
	  }
